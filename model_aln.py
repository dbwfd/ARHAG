...
import numpy as np
from model_harn import HeterogeneousGnn
import side



class TransZero(nn.Module):
    def __init__(self, config, att, init_w2v_att, att_index,seenclass, unseenclass,
        ...
        self.is_dualbranch=config.is_dualbranch
        ...
        if self.is_dualbranch:
            self.W_2 = nn.Parameter(nn.init.normal_(
                torch.empty(self.dim_v, config.tf_common_dim)), requires_grad=True)
        #self.W_2 = nn.Parameter(nn.init.normal_(torch.empty(self.dim_v, config.tf_common_dim)), requires_grad=True)
        # transformer
        ...
        #for hg
        self.atttribute_reason=HeterogeneousGnn(config=config,att_index=att_index)
    ...
    def forward(self, input, from_img=False):
        ...
        att_vf, att_confidence = self.forward_feature_transformer(Fs)
        if self.config.is_attreason:
            att_confidence=torch.max(att_confidence,dim=-1)[0]

            #att_confidence=torch.ones((40,85))
            #att_confidence[:,::2]=10

            att_vf_reasoned = self.forward_feature_attreason(att_vf,att_confidence)

            #att_vf_reasoned = self.forward_feature_attreason(att_vf)


        # embedding to semantic space

        if self.is_dualbranch and self.config.is_attreason:
            v2s_embed = torch.einsum('iv,vf,bif->bi', V_n, self.W_1, att_vf)
            v2s_embed_reasoned = torch.einsum('iv,vf,bif->bi', V_n, self.W_2, att_vf_reasoned)
            v2s_embed = (1 - self.config.alpha) * v2s_embed + self.config.alpha * v2s_embed_reasoned
        elif self.config.is_attreason:
            v2s_embed = torch.einsum('iv,vf,bif->bi', V_n, self.W_1, att_vf_reasoned)
        else:
            v2s_embed = torch.einsum('iv,vf,bif->bi', V_n, self.W_1, att_vf)
    ...
    def forward_feature_transformer(self, Fs):
        ...
        Trans_out, att_confidence = self.transformer(Fs, V_n)
        #embed = torch.einsum('iv,vf,bif->bi', V_n, self.W_1, Trans_out)
        return Trans_out, att_confidence
    def forward_feature_attreason(self,att_vf,att_confidence=None):
        if att_confidence is not None:
            att_vf=self.atttribute_reason(att_vf,att_confidence)
        else:
            att_vf = self.atttribute_reason(att_vf)
        # embed = torch.einsum('iv,vf,bif->bi', V_n, self.W_2, att_vf)
...
class Transformer(nn.Module):
    ...
    def forward(self, f_cv, f_attr):
        ...
        out, att_confidence = self.transformer_decoder(h_attr_batch.permute(1, 0, 2), memory)
        return out.permute(1, 0, 2), att_confidence
...
class TransformerDecoderLayer(side.TransformerDecoderLayer):
    ...
    def forward(self, tgt, memory, tgt_mask=None, memory_mask=None,
        ...
        tgt2, att_confidence = self.multihead_attn(tgt, memory, memory, attn_mask=memory_mask,
