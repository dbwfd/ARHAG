...
wandb.init(project='AttributeReason', config='wandb_config/awa2_czsl.yaml')
config = wandb.config
print('Config file from wandb:', config)

# load dataset
dataloader = AWA2DataLoader('../TransZero-train', config.device)
...
att_index=torch.from_numpy(np.load('att_index/att_index_AWA2.npy')).to(config.device)


# TransZero model
model = TransZero(config, att, init_w2v_att, att_index,
